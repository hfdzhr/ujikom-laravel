								...
								<tbody>
									@forelse ($carts as $cart)
										...
										<tr>
											...
											<td class="product-price-cart"><span class="amount">Rp. {{ number_format($cart->price) }}</span></td>
											<td class="product-quantity">
												<input type="number" name="items[{{ $cart->id }}][quantity]" min="1" required value="{{ $cart->quantity }}">
											</td>
											<td class="product-subtotal">Rp. {{ number_format($cart->price * $cart->quantity)}}</td>
											<td class="product-remove">
												<a href="" onclick="event.preventDefault();document.getElementById('delete-cart').submit();" class="delete"><i class="pe-7s-trash"></i></a>
											</td>
										</tr>
									@empty
										<tr>
						...
						<div class="row">
							<div class="col-md-5 ml-auto">
								<div class="cart-page-total">
									...
									<a href="{{ route('checkout.process') }}">Proses Checkout</a>
								</div>
							</div>
						</div>
				</div>
